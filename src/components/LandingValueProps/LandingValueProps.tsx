...
function LandingValuePropsComponent({ landingData, className = '' }: LandingValuePropsProps) {
  const { valueProps } = landingData;

  return (
    <SectionContainer
      sectionClassName={`px-4 pb-12 pt-12 md:px-6 md:pb-20 md:pt-20 lg:px-10 ${className}`.trim()}
    >
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:gap-8 xl:grid-cols-3">
        {valueProps.items.map((item) => (
          ...
          >
            ...
            <div
              aria-hidden="true"
              className="pointer-events-none absolute inset-0 bg-gradient-to-br from-slate-950/85 via-slate-900/65 to-slate-900/35"
            />
...
LandingValuePropsComponent.displayName = 'LandingValueProps';

export default memo(LandingValuePropsComponent);
